...
class CourseParser:
    ...
    def parse_additional_info(course_link: str) -> tuple:
        page = requests.get(
            urljoin(CourseParser.BASE_URL, f"{course_link}")
        ).content
    ...
    def parse_course(course_soup: Tag) -> list[Course]:
        ...
        for course_type, course_link in zip(
                course_type_list, course_link_list
        ):

            modules, topics, duration = (
                CourseParser.parse_additional_info(course_link)
            )
...
def get_all_courses() -> list[Course]:
    all_courses = []
    page = requests.get(CourseParser.BASE_URL).content
    soup = BeautifulSoup(page, "html.parser")
    ...
    [
        all_courses.extend(
            CourseParser.parse_course(course_soup)
