...
class CoursesScraper:
    ...
    def __init__(self) -> None:
        self.BASE_URL = "https://mate.academy/"

    def get_course_data(self, url: str) -> dict:
        course_url = urljoin(self.BASE_URL, url)
    ...
    def parse_single_part_time_course(self, soup: BeautifulSoup) -> Course:
        ...
        return Course(
            ...
            **self.get_course_data(part_time_url)
        )

    def parse_single_full_time_course(self, soup: BeautifulSoup) -> Course:
        ...
        return Course(
            ...
            **self.get_course_data(full_time_url)
        )
    ...
    def get_courses(self) -> [Course]:
        ...
        for course_data in courses_data:
            try:
                all_courses.append(
                    self.parse_single_part_time_course(course_data)
                )
            except TypeError:
                continue
            finally:
                all_courses.append(
                    self.parse_single_full_time_course(course_data)
                )
        return all_courses
