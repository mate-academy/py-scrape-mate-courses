...
class Course:
    ...
    def get_one_course(
            cls,
            course: BeautifulSoup, course_type: str) -> "Course":
        return cls(
            name=course.select_one(".typography_landingH3__vTjok").text,
            short_description=(
                course.select_one(".CourseCard_courseDescription__Unsqj").text
            ),
...
def get_courses(page_soup: BeautifulSoup, course_type: str) -> list[Course]:
    ...
    return [Course.get_one_course(course, course_type) for course in courses]
...
def get_all_courses() -> list[Course]:
    ...
    all_courses = (
        get_courses(soup, "full-time") + get_courses(soup, "part-time")
    )
