...
def get_detail_page(course_url: str) -> str:
    page = requests.get(course_url).content
    soup = BeautifulSoup(page, "html.parser")
    description = soup.select_one(
        ".AboutProfessionSection_secondaryBlock__7F__1 "
...
def parse_single_coursers(course_soup: BeautifulSoup,
                          course_type: str) -> list:
    courses = course_soup.select(".DropdownCoursesList_coursesListItem__5fXRO")
    parsed_courses = []
    for course in courses:
        course_title = course.select_one(
            ".Button_transparentLight__JIwOr"
        )["title"]
        course_link = course.select_one(
            ".Button_transparentLight__JIwOr"
        ).get("href")
        course_url = f"{URL}{course_link}"
        short_description = get_detail_page(course_url)

        parsed_courses.append(Course(name=course_title,
...
def get_all_courses() -> list[Course]:
    ...
    courses_full_time = soup.select(
        ".DropdownCoursesList_coursesList__xjALB"
    )[0]
    courses_part_time = soup.select(
        ".DropdownCoursesList_coursesList__xjALB"
    )[1]

    full_time_courses = parse_single_coursers(courses_full_time,
                                              CourseType.FULL_TIME)
    part_time_courses = parse_single_coursers(courses_part_time,
...
def write_to_csv(courses: Course) -> None:
